...
export const getproducts = async (req, res) => {
  try {
    ...
    if (rows.length <= 0) {
      return res.status(404).json({ message: "No hay products registrados" });
    }
...
export const postproducts = async (req, res) => {
  try {
    ...
    const [result] = await pool.query("SELECT MAX(id) AS last_id FROM products");
    // 2. Obtenemos el último ID y lo incrementamos
    const lastId = result[0].last_id || 0; // Si no hay products, el último ID será 0
    const newId = lastId + 1;  // Calculamos el nuevo ID
    // 3. Insertamos el nuevo producto con el ID incrementado
    const [insertResult] = await pool.query(
      "INSERT INTO products (id, nombre, descripcion, precio_costo, precio_venta, cantidad, fotografia) VALUES (?, ?, ?, ?, ?, ?, ?)",
...
export const putproducts = async (req, res) => {
  try {
    ...
    const [result] = await pool.query(
      "UPDATE products SET nombre = ?, descripcion = ?, precio_costo = ?, precio_venta = ?, cantidad = ?, fotografia = ? WHERE id = ?",
